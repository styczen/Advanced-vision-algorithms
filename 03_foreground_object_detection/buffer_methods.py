...
# INFO: fill buffer before background aproximation
...
def update_frame_buffer(current_frame, frame_buffer, iN):
    frame_buffer[:, :, iN] = current_img
    iN = iN + 1
    if iN == 60:
        iN = 0
    return frame_buffer, iN
...
# Frame buffer
frame_buffer = np.zeros((prev_frame.shape[0], prev_frame.shape[1], FRAME_BUFFER_SIZE), np.uint8)
iN = 0
...
for i in range(roi_start+1, roi_end, 1):
    ...
    frame_buffer, iN = update_frame_buffer(current_img, frame_buffer, iN)

    # bg_mask = median_bg(frame_buffer)
    bg_mask = mean_bg(frame_buffer)

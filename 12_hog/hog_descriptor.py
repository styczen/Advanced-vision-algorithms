...
def grad(img):
    ...
    gradRx = scipy.ndimage.filters.convolve1d(
        np.int32(img[:, :, 0]), np.array([-1, 0, 1]), 1)
    gradRy = scipy.ndimage.filters.convolve1d(
        np.int32(img[:, :, 0]), np.array([-1, 0, 1]), 0)

    gradGx = scipy.ndimage.filters.convolve1d(
        np.int32(img[:, :, 1]), np.array([-1, 0, 1]), 1)
    gradGy = scipy.ndimage.filters.convolve1d(
        np.int32(img[:, :, 1]), np.array([-1, 0, 1]), 0)

    gradBx = scipy.ndimage.filters.convolve1d(
        np.int32(img[:, :, 2]), np.array([-1, 0, 1]), 1)
    gradBy = scipy.ndimage.filters.convolve1d(
        np.int32(img[:, :, 2]), np.array([-1, 0, 1]), 0)
    ...
    for jj in range(0, YY_cell):
        for ii in range(0, XX_cell):
            ...
            for k in range(0, cellSize*cellSize):
                ...
                # Przedzial ma rozmiar 20, srodek to 20
                t0 = np.floor((t - 10) / 20) * 20 + 10
...
def HOGpicture(w, bs=8):  
    # w - histogramy gradientow obrazu, bs - rozmiar komorki (u nas 8)
    bim1 = np.zeros((bs, bs))
    bim1[np.round(bs//2):np.round(bs//2)+1, :] = 1
    bim = np.zeros(bim1.shape+(9,))
    bim[:, :, 0] = bim1
    for i in range(0, 9):  # 2:9
        bim[:, :, i] = scipy.ndimage.rotate(input=bim1,
                                            angle=-i*20,
                                            mode='nearest',
                                            reshape=False)/255
    Y, X, Z = w.shape
...
def hog(img):
    """Calculates histograms of oriented gradients."""
    gradient, orientation = grad(img)
    histogram = hist(gradient, orientation, img.shape[1], img.shape[0])
    hist_normalized = hist_norm(histogram, img.shape[1], img.shape[0])
